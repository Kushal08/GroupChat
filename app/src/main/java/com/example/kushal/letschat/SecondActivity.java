...
public class SecondActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mMessageRef = mFirebaseRef.child("chats");// connection to message object

        mEditText = (EditText)this.findViewById(R.id.message_text);
        mListView = (ListView)this.findViewById(R.id.listView);

        mListAdapter = new FirebaseListAdapter<Chat>(mMessageRef,Chat.class,R.layout.layout,this) {
            @Override
            protected void populateView(View v, Chat model) {
                ((TextView)v.findViewById(R.id.message_text_view1)).setText(model.getChat());
                ((TextView)v.findViewById(R.id.username_text_view1)).setText(model.getName()+ ": ");
    ...
    public void onSendButtonClick1(View view) {
        String message = mEditText.getEditableText().toString();//message to be sent
        if (!message.isEmpty()) {
            mMessageRef.push().setValue(new Chat(mUsername, message));
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_settings) {
            ...
            new SecondActivity.AsyncLogin().execute(email);
            return true;
        }
    ...
    {
        ProgressDialog pdLoading = new ProgressDialog(SecondActivity.this);
